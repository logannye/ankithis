...
class ContentProfile(UUIDMixin, TimestampMixin, Base):
    ...
    information_density = sa.Column(
        sa.Enum(InformationDensity),
        nullable=False,
        default=InformationDensity.MODERATE,
    )
    structure_quality = sa.Column(
        sa.Enum(StructureQuality),
        nullable=False,
        default=StructureQuality.SEMI_STRUCTURED,
    )
    primary_knowledge_type = sa.Column(
        sa.Enum(KnowledgeType),
        nullable=False,
        default=KnowledgeType.MIXED,
    )
